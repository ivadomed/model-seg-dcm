...
def create_nnunet_from_plans(
    ...
    if initialization == "scratch":
        ...
        assert pretrained_checkpoint is not None, "Please provide the path to the pretrained checkpoint."
        logger.info(f"Loading weights from {pretrained_checkpoint} ...")
        pretrained_weights = torch.load(pretrained_checkpoint)

        # remove the segmentation layers (the 1x1(x1) layers that produce the segmentation maps) 
        # identified by keys with '.seg_layers')
        pretrained_weights = {k: v for k, v in pretrained_weights.items() if '.seg_layers' not in k}
...
if __name__ == "__main__":
    ...
    model = create_nnunet_from_plans(nnunet_plans, 1, 1, enable_deep_supervision, "scratch")
    input = torch.randn(1, 1, 64, 192, 320)
    # M1: using encoder and decoder separately
    skips = model.encoder(input)
    print(skips[-1].shape)
    output = model.decoder(skips)
    # # M2: using the whole model directly
    # output = model(input)
    ...
    # # load the model
    # model = create_nnunet_from_plans(nnunet_plans, 1, 1, enable_deep_supervision, "pretrained", "model.pth")
