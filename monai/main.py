...
def get_args():
    parser = argparse.ArgumentParser(description='Script for (pre)training lesion segmentation model for DCM and SCI patients.')
    
    # arguments for model
    parser.add_argument('-m', '--model', choices=['swinunetr', 'nnunet', 'mednext', 'swinunetr'], 
...
class Model(pl.LightningModule):
    def __init__(self, config, data_root, net, loss_function, optimizer_class, exp_id=None, results_path=None):
        ...
        # On the other hand, ivadomed used a patch-size that's heavily cropped along the R-L direction so that 
        ...
        self.patch_size = config["preprocessing"]["patch_size"]
