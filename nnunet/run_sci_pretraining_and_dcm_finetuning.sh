...
dataset_num_ptr="190"
dataset_name_ptr="Dataset${dataset_num_ptr}_tSCI3SitesALSeed710Pretrain"
...
echo "-------------------------------------------------------"
echo "Running plan_and_preprocess for ${dataset_name_ftu} ... (Finetuning dataset)"
echo "-------------------------------------------------------"
nnUNetv2_plan_and_preprocess -d ${dataset_num_ftu} --verify_dataset_integrity -c ${configuration}

echo "-------------------------------------------------------"
echo "Running plan_and_preprocess for ${dataset_name_ptr} ... (Pretraining dataset)"
echo "-------------------------------------------------------"
nnUNetv2_plan_and_preprocess -d ${dataset_num_ptr} --verify_dataset_integrity -c ${configuration}

echo "-------------------------------------------------------"
echo "Extracting dataset fingerprint for ${dataset_name_ptr} ... (Pretraining dataset)"
echo "-------------------------------------------------------"
nnUNetv2_extract_fingerprint -d ${dataset_num_ptr}

echo "-------------------------------------------------------"
echo "Moving plans from ${dataset_name_ftu} to ${dataset_name_ptr} ... (Finetuning --> Pretraining)"
echo "-------------------------------------------------------"
nnUNetv2_move_plans_between_datasets -s ${dataset_num_ftu} -t ${dataset_num_ptr} -sp nnUNetPlans -tp nnUNetMovedPlans

echo "-------------------------------------------------------"
echo "Running (only) preprocessing for ${dataset_name_ptr} after moving plans ... (Pretraining dataset)"
echo "-------------------------------------------------------"
