"""
Convert BIDS-structured datasets (dcm-zurich) to the nnUNetv2 dataset format.
...
def get_parser():
    ...
    parser.add_argument('--dataset-name', '-dname', default='DCMcompression', type=str,
...
def find_site_in_path(path):
    ...
    """
    # Find 'dcm-zurich'
    match = re.search(r'dcm-zurich?', path)
...
def main():
    ...
    all_files, train_images, test_images = [], {}, {}
    ...
    for dataset in args.path_data:
        ...
        # get recursively all GT compression labels
        label_files = [str(path) for path in root.rglob('*_label-compression-manual.nii.gz')]

        # add to the list of all subjects
        all_files.extend(label_files)

        # Get the training and test splits
        tr_subs, te_subs = train_test_split(label_files, test_size=test_ratio, random_state=args.seed)
    ...
    for subject_label_file in tqdm(all_files, desc="Iterating over all images"):

        # Construct path to the background image
        subject_image_file = subject_label_file.replace('/derivatives/labels', '').replace('_label-compression-manual', '')
